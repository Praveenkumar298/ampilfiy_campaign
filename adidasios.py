...
from spicipher import af_cipher,get_dlsdk_af_sig,get_cdn_token,get_gcd_sdk_af_sig,update_ios_cksm_v2,get_ios_skadsdk_sign
...
campaign_data = {
   ...
   'bundle_name' : 'adidas',
   'app_id' : '1266591536',
   'app_version_code' : '2023.7.24.12.33',
   'app_version_name' : '5.26.2',
   'supported_os' : '15',
   'platformextension' : 'ios_mparticle',
   'ref' : 'db7329eef4753e02f2f61ae54182f260365b4da62dc7cab86fc41cde012bd3f8;0;0;0;0;0',
    'appsflyer' : { 
      'key' : 's25jKcTx5TJrcoUbmd2gLo',
      'server_api_version' : '6.10',
      'buildnumber' : '6.10.0',
      'version' : 'v6.10',
      'spi_key' : 'ls4Y7BiLWJk2IGt39pwOxUs/+o2b+IZis3FQhoFq5F4ZgE1tNWiMPHdX5FlPw3Cr',
    #    'spi_key' : '+mdy0Qxno2OLf+9BK8v7esLnxW8iefCTF+q9n1dWkPqmHS9XXnUZazfNd95N2idB',#devkey
    },
...
firebase_data = {"run": true, "X-Android-Cert": "AIzaSyC0B5ETKypPKinGG7gHiohXkEwUmkOR9oM", "data": {"appId": "1:475851980505:ios:d8d5a06a8d7254be", "fid": "dk4Uw1XMmEE5gSWr6p8P-c", "authVersion": "FIS_v2", "sdkVersion": "i:10.7.0"}, "agent": "adidas/2023.7.24.12.33 CFNetwork/1335.0.3.2 Darwin/21.6.0", "url": "/v1/projects/adidas-app-production/installations/"}
#=======================conversion===================================
...
def install(app_data, device_data):
    ...
    if random.choice(poss):
        req  = appsflyer_inapps( app_data, device_data,event__Name="af_complete_registration", event__Value={})
        util.execute_request(**req)
        ...
        random.shuffle(poss)
        if random.choice(poss):
            req  = appsflyer_inapps( app_data, device_data,event__Name="af_login_start", event__Value={})
            util.execute_request(**req)

        req = appsflyer_launch(app_data, device_data)
        util.execute_request(**req)

        req  = appsflyer_inapps( app_data, device_data,event__Name="af_content_view", event__Value={})
        util.execute_request(**req)

        poss = [True]*20+[False]*80
        random.shuffle(poss)
        if random.choice(poss):
            req  = appsflyer_inapps( app_data, device_data,event__Name="af_add_to_cart", event__Value={})
            util.execute_request(**req)

            poss = [True]*20+[False]*80
            random.shuffle(poss)
            if random.choice(poss):
                req  = appsflyer_inapps( app_data, device_data,event__Name="af_initiated_checkout", event__Value={})
                util.execute_request(**req)

                # app_data['oid']=str(random.randint(61246646,61246648))

                poss = [True]*10+[False]*90
                random.shuffle(poss)
                if random.choice(poss):
                    req  = appsflyer_inapps( app_data, device_data,event__Name="af_purchase", event__Value= {})
                    util.execute_request(**req)
                    app_data['af_purchase']=True
...
def open( app_data, device_data, day=1 ):
    if app_data.get('times'):
        ...
        if not app_data.get('register') and day <3:
            ...
            if random.choice(poss):
                ...
                req  = appsflyer_inapps( app_data, device_data,event__Name="Register Normal", event__Value={"status":True,"whatsapp consent":False,"method":"email"})
                util.execute_request(**req)
            ...
            pos = [True]*60+[False]*40
            random.shuffle(pos)
            if random.choice(pos):

                req  = appsflyer_inapps( app_data, device_data,event__Name="af_login_start", event__Value={})
                util.execute_request(**req)

                if app_data.get('register'):
                    req  = appsflyer_inapps( app_data, device_data,event__Name="af_login", event__Value={})
                    util.execute_request(**req)
                    app_data['register'] = True
                ...
                if random.choice(poss):
                    ...
                    if random.choice(poss):
                        ...
                        if not app_data.get('af_purchase'):
