...
campaign_data = {
   ...
   'app_name' : 'Almowafir',
   'bundle_name' : 'Almowafir',
   'app_id' : '1480507684',
   'app_version_code' : '0',
   'app_version_name' : '1.4.4',
   'supported_os' : '13',
   'platformextension' : 'ios_cordova',
   'ref' : 'db7329eef4753e02f2f61ae54182f260365b4da62dc7cab86fc41cde012bd3f8;0;0;0;0;0',
    'appsflyer' : { 
      ...
      'buildnumber' : '6.8.1',
      'version' : 'v6.8',
      'spi_key' : 'SLOhNg2GbBhw4Tmq6BMgNUYD8mL5vmZJuZqGAJrU7Ph900Rb+szCs9TOEDZ1HAdB',
    #    'spi_key' : 'R3P7NNK0TVCO+vHL+qKiMhY727qLRce2H9EulCoW9WBgIEaOUU7ctDU17vuYR0g6',#devkey
    },
...
def appsflyer_dlsdk(app_data,device_data):
    ...
    params = {
        'af_sig':'f6846eba1ab2adc173130d28b90351a42a1fa34057579a355a207193c60c3cb5',
...
def install(app_data, device_data):
    ...
    if app_data.get('register'):
        ...
        if random.choice(poss):
            name = 'GetCoupon'
            val = {"BrandName": "Magrabi", "page": "HomePage"}
            req = appsflyer_inapps(app_data, device_data, event__Name = name, event__Value = val)
            util.execute_request(**req)


            name = 'Intent_1stSession'
            val = {"BrandName": "Magrabi", "page": "HomePage"}
            req = appsflyer_inapps(app_data, device_data, event__Name = name, event__Value = val)
            util.execute_request(**req)


            name = 'InterestAction_Old'
            val = {"CouponCode": "ALM15", "BrandName": "Magrabi", "Action": "Copy", "type": "coupon"}
            req = appsflyer_inapps(app_data, device_data, event__Name = name, event__Value = val)
            util.execute_request(**req)
            ...
            name = 'GoToStore'
            val = {"type": "coupon", "BrandName": "Magrabi", "CouponCode": "ALM15"}
            req = appsflyer_inapps(app_data, device_data, event__Name = name, event__Value = val)
            util.execute_request(**req)


            name = 'LinkOut_Old'
            val = {"type": "coupon", "BrandName": "Magrabi", "Action": "GoToStore"}
            req = appsflyer_inapps(app_data, device_data, event__Name = name, event__Value = val)
            util.execute_request(**req)
...
def open( app_data, device_data, day=1 ):
    if app_data.get('times'):
        ...
        if not app_data.get('register') and day < 7:
