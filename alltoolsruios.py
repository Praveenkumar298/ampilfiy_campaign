...
campaign_data = {
    'package_name'       :'com.notissimus.VseInstrumenti.iPhone',
    'app_size'           : 100,
    'app_name'           :'vseinstrumenti',
    'app_version_name'   : '2.45.1',
    'app_version_code'   : '4051',
    'app_id'             : '963087214',
...
def install(app_data, device_data):
    ...
    if random.choice(poss):
        #Add_to_cart
        req= adjust_event(campaign_data, app_data, device_data,event_token='en9je1',t1=10,t2=20,callback_params=json.dumps({"product_id":"25338a9b-8ef4-4a05-bbc7-632d60297828","rating":"3.8","reviews":"163","items":"[{item_name: Бесщеточный аккумуляторный импульсный гайковерт AEG BSS18C12ZBL LI-402C 4935459427; currency: RUB; item_brand: AEG; item_category: Аккумуляторные гайковерты; quantity: 1; item_id: 15703839; price: 27304; is_on_sale: false}]","from":"main"}))
        ...
        if random.choice(poss):
            #view_cart
            req= adjust_event(campaign_data, app_data, device_data,event_token='snkjc6',t1=10,t2=20,callback_params=json.dumps({"product_id":"25338a9b-8ef4-4a05-bbc7-632d60297828","rating":"3.8","reviews":"163","items":"[{item_name: Бесщеточный аккумуляторный импульсный гайковерт AEG BSS18C12ZBL LI-402C 4935459427; currency: RUB; item_brand: AEG; item_category: Аккумуляторные гайковерты; quantity: 1; item_id: 15703839; price: 27304; is_on_sale: false}]","from":"main"}))
            util.execute_request(**req)
            ...
            if random.choice(poss):
                #click_on_purchase
                req= adjust_event(campaign_data, app_data, device_data,event_token='ej6pm2',t1=10,t2=20,callback_params=json.dumps({"product_id":"25338a9b-8ef4-4a05-bbc7-632d60297828","rating":"3.8","reviews":"163","items":"[{item_name: Бесщеточный аккумуляторный импульсный гайковерт AEG BSS18C12ZBL LI-402C 4935459427; currency: RUB; item_brand: AEG; item_category: Аккумуляторные гайковерты; quantity: 1; item_id: 15703839; price: 27304; is_on_sale: false}]","from":"main"}))
                util.execute_request(**req)
                app_data['purchase'] = True
