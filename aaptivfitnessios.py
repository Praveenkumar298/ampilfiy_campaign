...
null = None
campaign_data = {
   'package_name' : 'com.skyfitsports.skyfit',
   'app_name' : 'SkyFit',
   'bundle_name' : 'Aaptiv',
   'app_id' : '869058995',
   'app_version_code' : '150',
   'app_version_name' : '13.0.10',
   'supported_os' : '15',
   'platformextension' : 'ios_segment',
   'ref' : 'db7329eef4753e02f2f61ae54182f260365b4da62dc7cab86fc41cde012bd3f8;0;0;0;0;0',
    'appsflyer' : { 
      'key' : 'kt84VbFxJURX7cMe8aHH2H',
      'server_api_version' : '6.12',
      'buildnumber' : '6.12.0',
      'version' : 'v6.12',
      'spi_key' : 'zsJkt7You+TZax2eeU6Gy344DM0Y8EknXSOsmvHRCMNLJjo7CM6GOb4kjy/P61AG',
    #    'spi_key' : 'MkPxpYL3mqaaQ7Z7v7XM6VXfkBF6v3ITWx48rrhV3eomC/yrDkZ7Z9H1C10ORxSO',#devkey
    },
...
firebase_data = {"run": true, "X-Android-Cert": "AIzaSyBoSHS74svcSW_xuM87ZkVy_NrqW1n4J9o", "data": {"appId": "1:992708705801:ios:333f684542be1e53", "fid": "diGfiMaaD0TbgisjTKeGS0", "authVersion": "FIS_v2", "sdkVersion": "i:8.15.0"}, "agent": "SkyFit/150 CFNetwork/1335.0.3.2 Darwin/21.6.0", "url": "/v1/projects/aaptiv-prod/installations/"}
#=======================conversion===================================
def appsflyer_conversion(app_data, device_data):
    url = 'https://8rzcbk-conversions.appsflyersdk.com/api/'+campaign_data.get('appsflyer').get('version') +'/iosevent'
    httpmethod ='post'
    headers = {
...
def appsflyer_attr(app_data, device_data):
    url = 'https://8rzcbk-attr.appsflyersdk.com/api/'+campaign_data.get('appsflyer').get('version') +'/iosevent'
    httpmethod = 'post'
    headers = {
        'Content-Type': 'application/octet-stream',
        'Accept': '*/*',
...
def appsflyer_launch(app_data, device_data):
    url = 'https://8rzcbk-launches.appsflyersdk.com/api/'+campaign_data.get('appsflyer').get('version') +'/iosevent'
    httpmethod = 'post'
    headers = {
        'Content-Type': 'application/octet-stream',
        'Accept': '*/*',
...
def appsflyer_inapps(app_data, device_data, event__Name ='', event__Value =''):
    url = 'https://8rzcbk-inapps.appsflyersdk.com/api/'+campaign_data.get('appsflyer').get('version')+'/iosevent'
    httpmethod = 'post'
    headers = {
        'Content-Type': 'application/octet-stream',
        'Accept': '*/*',
...
def appsflyer_gcdsdk(app_data, device_data):
    url = 'https://8rzcbk-gcdsdk.appsflyersdk.com/install_data/v5.0/id' + campaign_data.get('app_id')
    httpmethod = 'get'
    headers = {
        'Accept': '*/*',
...
def appsflyer_sdadsdk( app_data, device_data ):
    ...
    url = 'https://8rzcbk-skadsdk.appsflyersdk.com/api/v1.0/rules'
    headers = {
        'Accept-Encoding': "gzip, deflate, br",
        'User-Agent': '{0}/{1} CFNetwork/{2} Darwin/{3}'.format(campaign_data.get('app_name'), campaign_data.get('app_version_code'), device_data.get('cfnetwork').split('_')[0], device_data.get('cfnetwork').split('_')[1]),
...
def dyanmic_config( app_data, device_data ):
    method = 'post'
    currency=util.country_list.get(device_data.get('locale').get('country').upper()).get('currency')
    url = 'https://8rzcbk-dynamic-config-api.appsflyer.com/api/dynamic-config/1'
    headers = {
        'Accept-Encoding': "gzip, deflate, br",
        'User-Agent': '{0}/{1} CFNetwork/{2} Darwin/{3}'.format(campaign_data.get('app_name'), campaign_data.get('app_version_code'), device_data.get('cfnetwork').split('_')[0], device_data.get('cfnetwork').split('_')[1]),
...
def appsflyer_cdn( app_data, device_data ):
    ...
    url = 'https://8rzcbk-cdn-settings.appsflyersdk.com/ios/v1/'+str(app_data.get('cdn_token'))+'/settings'
    headers = {
        'Content-Type': 'application/json',
        'User-Agent': '{0}/{1} CFNetwork/{2} Darwin/{3}'.format(campaign_data.get('app_name'), campaign_data.get('app_version_code'), device_data.get('cfnetwork').split('_')[0], device_data.get('cfnetwork').split('_')[1]),
...
def open( app_data, device_data, day=1 ):
    if app_data.get('times'):
        ...
        if not app_data.get('login') and day < 7:
