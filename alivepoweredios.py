...
campaign_data = {
    'package_name'       :'com.aia.rn.th.2020.dp01a',
    'app_size'           : 100,
    'app_name'           :'ALive',
    'app_version_name'   : '1.69.0',
    'app_version_code'   : '1',
    'app_id'             : '1544889543',
    ...
    'tracker'            : ['Adjust'],
    'adjust'         : {
        'app_token'     : '8mt8ar2byfsw',
        'sdk'           : 'react-native4.29.4@ios4.29.5',
        },
...
def install(app_data, device_data):
    ...
    if random.choice(poss):
        #START_NONE_INSURANCE
        req =adjust_event(campaign_data, app_data, device_data,t1=0,t2=0,event_token='j903zc')
        util.execute_request(**req)

        poss = [True]*50+[False]*50
        random.shuffle(poss)
        if random.choice(poss):
            #COMPLETE_NONE_INSURANCE
            req =adjust_event(campaign_data, app_data, device_data,t1=0,t2=0,event_token='c3pl6l')
            util.execute_request(**req)
            app_data['non_insurance'] = True
    ...
    if random.choice(poss):
        #START_INSURANCE
        req =adjust_event(campaign_data, app_data, device_data,t1=0,t2=0,event_token='kqf2ud')
        util.execute_request(**req)
        ...
        if random.choice(poss):
            #SUBMIT_INSURANCE
            req =adjust_event(campaign_data, app_data, device_data,t1=0,t2=0,event_token='4g1lvi')
            util.execute_request(**req)
            ...
            if random.choice(poss):
                #COMPLETE_INSURANCE
                req =adjust_event(campaign_data, app_data, device_data,t1=0,t2=0,event_token='jpvq9w')
                util.execute_request(**req)
                app_data['insurance'] = True
...
def open(app_data, device_data,day=1):
    if app_data.get('times'):
        ...
        #---------------------------------  
        if not app_data.get('non_insurance'): 
            poss = [True]*80+[False]*20
            random.shuffle(poss)
            if random.choice(poss):
                #START_NONE_INSURANCE
